...
from app.utils import e, parse_services, parse_portfolio, parse_testimonials, parse_faq, default_seo_title, chunks, convert_uuids_to_strings, build_request_payload
...
def register(dp, bot):
    ...
    def format_request_card(rec: dict, show_private: bool = True) -> str:
        data = json.loads(rec["site_params_json"] or "{}")
        site = data.get("site", {})
        client_json = data.get("client", {})
        ...
        has_seo = bool(seo.get("description"))
        has_hero = bool(hero.get("title"))
    ...
    async def cmd_my_requests(message: types.Message, state: FSMContext):
        ...
        if not get_user_by_tgid(message.from_user.id):
            kb = types.ReplyKeyboardMarkup(resize_keyboard=True); kb.add(BTN_NEW, BTN_ADMIN_LOGIN)
    ...
    async def cb_open_request(call: types.CallbackQuery):
        ...
        try:
            ...
            site = json.loads(rec["site_params_json"]).get("site",{})
    ...
    async def on_edit_value(message: types.Message, state: FSMContext):
        ...
        site_json = data_json.setdefault("site", {})
    ...
    async def cb_export_one(call: types.CallbackQuery):
        ...
        payload = convert_uuids_to_strings(build_request_payload(rec))
    ...
    async def cb_generate_site(call: types.CallbackQuery):
        ...
        payload = convert_uuids_to_strings(build_request_payload(rec))
        await call.answer()
        await post_generate_site(call.message.chat.id, payload, call.message)
