...
from app.constants import (
    ...
    EDITABLE_FIELDS, EDIT_HINTS, STATUS_LABELS,
...
from app.db import (
    ...
    get_request_payload, set_request_status,
)
...
def format_request_card(rec: dict, show_private: bool = True) -> str:
    ...
    from app.constants import PHOTO_CATEGORIES
    photo_cats = {}
    for img in images:
    ...
    if services:
        services_list = []
        for s in services[:5]:  # Показываем максимум 5
            name = e(s.get('name', ''))
    ...
    # Контактные данные для сайта (разделены)
    phone = site.get("phone") or site.get("site_contacts") or "—"
    email = site.get("email") or "—"
    address = site.get("address") or "—"

    # Блок клиента (приватный)
    client_block = ""
    if show_private:
    ...
    return (
        f"📋 <b>Заявка #{str(rec['id'])[:8]}...</b>\n"
        f"Статус: {status}\n\n"
        f"{client_block}"
        f"<b>🏢 Информация для сайта</b>\n"
        f"Компания: <b>{e(site.get('company'))}</b>\n"
        f"Сфера: {e(site.get('business_type'))}\n"
        ...
        f"• FAQ: {len(faq)}\n"
        f"• SEO: {'✅' if has_seo else '—'}\n"
        f"• Заголовки: {'✅' if has_hero else '—'}"
    )
...
def register(dp, bot):
    ...
    async def cb_open_request(call: types.CallbackQuery):
        ...
        try:
            ...
            await call.message.answer(f"⚠️ Ошибка при открытии заявки.")
    ...
    async def cb_delete_request(call: types.CallbackQuery, state: FSMContext):
        ...
        if ok:
            await call.message.edit_text(f"✅ Заявка удалена.")
        else:
    ...
    async def on_edit_value(message: types.Message, state: FSMContext):
        ...
        if field == "structure":
            ...
            if not services:
                return await message.answer("❌ Не удалось распознать услуги. Используйте формат: <i>Название — описание — цена</i>")
            ...
            site_json["address"] = val
        elif field == "work_hours":
            if len(val) < 3:
                return await message.answer("❌ График работы слишком короткий.")
    ...
    async def cb_export_one(call: types.CallbackQuery):
        ...
        # Используем io.BytesIO вместо файла
    ...
    async def cb_generate_site(call: types.CallbackQuery):
        ...
        if not has_min_requirements(payload):
            ...
            if not has_company:
                ...
                return await call.answer("❌ Сначала укажите название компании.", show_alert=True)
            else:
