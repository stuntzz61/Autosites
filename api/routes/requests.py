...
async def generate_site(request_id: str, user: dict = Depends(get_current_user)):
    ...
    if not webhook_url:
        ...
        try:
            ...
            manager_id_str = ""
            if user_id:
                manager_id_str = str(user_id)
                try:
                    manager = await db.get_user_by_id(manager_id_str)
            ...
            # Ensure structure includes "Карта" if not present
            structure = site_data.get('structure', [])
            if 'Карта' not in structure:
                structure = structure + ['Карта']
            ...
            webhook_data = {
                ...
                "manager_tg_id": manager_tg_id,  # Telegram user ID for notifications
                "chat_id": chat_id,  # Telegram chat ID where request was created
